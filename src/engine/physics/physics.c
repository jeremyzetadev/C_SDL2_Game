...
void physics_init(void){
    ...
    state.static_body_list = arraylist_create(sizeof(Static_body), 0);
...
size_t physics_static_body_create(vec2 position, vec2 size){
    ...
    if(arraylist_append(state.static_body_list, &body) == -1){
        ERROR_RETURN(-1,"Could not append static body to list\n");
    }
...
Static_body *physics_static_body_get(size_t index){
    return arraylist_get(state.static_body_list, index);
}
