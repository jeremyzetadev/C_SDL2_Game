...
void render_init_shaders(Render_State_Internal *state){
    ...
    glad_glUseProgram(state->shader_default);
    glad_glUniformMatrix4fv(
            glad_glGetUniformLocation(state->shader_default, "projection"),
            1,
            GL_FALSE,
...
void render_init_color_texture(u32 *texture){
    ...
    glad_glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, solid_white);
